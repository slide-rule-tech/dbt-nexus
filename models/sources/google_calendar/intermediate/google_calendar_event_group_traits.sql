...
-- Intermediate layer: Group traits for Google Calendar events
-- This layer extracts and standardizes group trait/attribute fields using Nexus macros

WITH organizer_domain_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['nexus_event_id', 'organizer.domain', "'domain'", "'organizer_domain'"]) }} as group_trait_id,
...
creator_domain_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['nexus_event_id', 'creator.domain', "'domain'", "'creator_domain'"]) }} as group_trait_id,
        nexus_event_id as event_id,
...
attendee_domain_traits AS (
    SELECT
        {{ create_nexus_id('group_trait', ['base.nexus_event_id', 'attendee.domain', "'domain'", "'attendee_domain'"]) }} as group_trait_id,
