...
domain_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['event_id', 'domain', "'domain'", "'sender_domain'"]) }} as group_trait_id,
        event_id,
...
domain_internal_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['event_id', 'domain', "'internal'", "'sender_domain'"]) }} as group_trait_id,
        event_id,
        'domain' as identifier_type,
        domain as identifier_value,
...
redirected_domain_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['event_id', redirected_domain('domain'), "'domain'", "'recipient_domain'"]) }} as group_trait_id,
        event_id,
        'domain' as identifier_type,
...
redirected_domain_internal_traits AS (
    SELECT 
        {{ create_nexus_id('group_trait', ['event_id', redirected_domain('domain'), "'internal'", "'recipient_domain'"]) }} as group_trait_id,
        event_id,
        'domain' as identifier_type,
        {{ redirected_domain('domain') }} as identifier_value,
        'internal' as trait_name,
