{{ config(materialized='view', tags=['realtime', 'identity-resolution', 'memberships']) }}

with membership_identifiers as (
    SELECT 
        * 
    FROM {{ ref(var('realtime_membership_identifiers_model')) }} -- DIRECTIVE: inject alasql sql table=source_membership_identifiers
),
